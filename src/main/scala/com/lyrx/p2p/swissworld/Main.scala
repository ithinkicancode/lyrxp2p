...
object Main {
  ...
  def showChange(props: AppProps) = (() => Main.toPage(props, Change.page))

  def showTomorrow(props: AppProps) = (() => Main.toPage(props, Tomorrow.page))

  def showWhereWeAre(props: AppProps) = (() => Main.toPage(props, WhereAreWe.page))
  ...
  def doRenderElements(elements: SlinkyElements): Unit = {
    ...
    ReactDOM.render(Footer(elements.appProps), elements.footer)
